...
class RandomAccessFileImpl implements RandomAccessFile {
  ...
  final ({
    ...
    void Function(int length) truncate,
  })
  _node;

  late final DB db = fileSystem.db;

  RandomAccessFileImpl(this.fileSystem, this.path, this._mode, this._node) {
    switch (_mode) {
    ...
    switch (_mode) {
      ...
      default:
        throw io.FileSystemException(
          '$operation failed',
          path,
          common.badFileDescriptor(path).osError,
        );
    }
  }
  ...
  Future<R> _asyncWrapper<R>(R Function() f) async {
    ...
    try {
      return await Future<R>.delayed(Duration.zero, () {
  ...
  Uint8List readSync(int bytes) {
    ...
    final copy = _node.bytes.sublist(_position, end);
    _position = end;
    return copy;
  }
  ...
  int readIntoSync(List<int> buffer, [int start = 0, int? end]) {
    ...
    for (i = start; i < end && _position < length; i += 1, _position += 1) {
      buffer[i] = _node.bytes[_position];
    }
    return i - start;
  }
  ...
  void setPositionSync(int position) {
    ...
    if (position < 0) {
      throw io.FileSystemException(
        'setPosition failed',
        path,
        common.invalidArgument(path).osError,
      );
    }
  ...
  void writeFromSync(List<int> buffer, [int start = 0, int? end]) {
    ...
    _node.bytes.setRange(_position, endPosition, buffer, start);
